...
QOpenGLShaderProgram* MythOpenGLPainter::GetProceduralShader(ProcSource Source, const QString& SourceHash)
{
    if (!m_render)
        return nullptr;
    ...
    // Temporary hack
    QStringList shaders = QString(*Source).split("HACK_HACK");
    if (shaders.size() != 2)
        return nullptr;

    auto result = m_render->CreateShaderProgram(shaders[0].trimmed(), shaders[1].trimmed());
